  ...
  async processDataUrl(dataUrl: string, originalName?: string, bumpCounters: boolean = true) {
    ...
    const doWork = async () => {
      ...
      try {
        ...
        console.warn('[CameraPage2] Failed to upload image to Firebase Storage', e);
      }
      ...
      try {
        ...
        console.warn('[CameraPage2] Failed to upload boxed image to Firebase Storage', e);
      }
    ...
    try {
      ...
      if (err && err.message === 'processing-timeout') {
        ...
        try {
          ...
          console.warn('[CameraPage2] Failed to upload timeout fallback image to Firebase Storage', e);
        }
        try {
          await this.imageStorage.addImage(entry, this.selectedSessionId || undefined);
          try {
