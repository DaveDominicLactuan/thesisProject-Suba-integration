...
private startUserSyncInBackground(userId: string): void {
  ...
  private async clearAllUserData(userId: string): Promise<void> {
    console.log('[BrowseFilePage.clearAllUserData] Beginning complete user data cleanup', { userId });
    ...
    for (const key of localStorageKeys) {
      try {
        ...
        console.warn(`[BrowseFilePage.clearAllUserData] Failed to remove localStorage key: ${key}`, e);
      }
    }
    ...
    for (const key of sessionStorageKeys) {
      try {
        ...
        console.warn(`[BrowseFilePage.clearAllUserData] Failed to remove sessionStorage key: ${key}`, e);
      }
    }
    ...
    if (userId) {
      ...
      for (const key of userSpecificKeys) {
        try {
          ...
          console.warn(`[BrowseFilePage.clearAllUserData] Failed to remove user-specific key: ${key}`, e);
        }
      }
    }
    ...
    try {
      if (this.imageStorage && typeof this.imageStorage.clear === 'function') {
        ...
        console.log('[BrowseFilePage.clearAllUserData] Cleared all images from storage');
      }
    } catch (e) {
      console.warn('[BrowseFilePage.clearAllUserData] Failed to clear images', e);
    }
    ...
    try {
      if ((this.imageStorage as any)._storage) {
        ...
        console.log('[BrowseFilePage.clearAllUserData] Cleared all sessions from storage');
      }
    } catch (e) {
      console.warn('[BrowseFilePage.clearAllUserData] Failed to clear sessions', e);
    }

    console.log('[BrowseFilePage.clearAllUserData] Complete user data cleanup finished');
  }
